...
import Brick.Types (BrickEvent (..), Context (availHeight, availWidth), EventM, Location (..), Result (image), Size (Greedy), Widget (..), getContext)
import Brick.Util (on)
import Brick.Widgets.Border (hBorder)
import Brick.Widgets.Border.Style (unicodeRounded)
import Brick.Widgets.Center (centerLayer, hCenterLayer)
import Brick.Widgets.Core (Padding (Pad), addResultOffset, fill, hBox, hLimitPercent, joinBorders, modifyDefAttr, padTop, str, vBox, withAttr, withBorderStyle, (<+>))
...
data Event
  ...
  | -- | Trigger eviction for the given scope (the sentinel @"*"@ for the whole project, a bare unit name, or a
    -- unit paired with a specific module), computed by 'TaskTree.selectedEvictTarget' (the project view now
    -- owns eviction targeting since the bytecode browser panel was merged into it).
...
drawUI State{..} =
  ( case _currentFocus of
    ...
    -- The Haskell-logo banner (see 'drawHaskellArt') is drawn on every screen, not just the idle one: on the
    -- main session view it overlaps the active-tasks panel in the top-right corner, the same way it sits above
    -- the idle screen's start-server form. On the idle\/start screen, the "GHC" banner, the operational log and
    -- the start-server form are additionally rendered as their own independent layers (see 'drawIdleLog',
    ...
    -- to leave behind.
    ++ [drawHaskellArt]
    ++ maybe [drawIdleLog _opLog, drawIdleStartServer _serveForm] (const []) session
    ++ [ vBox
          [ joinBorders $
...
drawHaskellArt = hAnchorRightLayer 2 $ padTop (Pad 2) $
  vBox ((hBox . fmap renderArtCell <$> haskellLogo) ++ [
    str " ",
    hBox [
      withAttr haskellLogoArrowAttr (str "   G H C"),
      withAttr haskellLogoLambdaAttr (str "      S E R V E R")
    ]
  ])
...
handleEvent (AppEvent (RequestEvict unitId modName)) = do
  ...
  zoom (currentSession . Session.taskTree) (TaskTree.handleEvent (TaskTree.EvictedBco unitId modName))
...
handleEvent (AppEvent (CleanCompleted target success)) =
  when success $ do
    ...
    zoom (currentSession . Session.activeTasks) (ActiveTasks.addSeparator ("Cleaned " ++ Text.unpack target))
      ...
      V.EvKey (V.KChar 'x') [] -> do
        ...
        case (mtree >>= TaskTree.selectedExecuteTarget, mfirst) of
          ...
          (_, Nothing) -> do
            ...
            beep
      V.EvKey (V.KChar 'e') [] | current == TaskTree -> do
        mtree <- preuse (currentSession . Session.taskTree)
    ...
    , appAttrMap =
        const $
          attrMap
            ...
            , (listSelectedAttr, V.defAttr `V.withBackColor` brightBlack)
            , -- The cursor's background is the terminal's own "black" (not "brightBlack"\/blue), which reads
              -- as too strong against Solarized-family palettes where the bright variants are mapped to
              -- vivid accent colors; no foreground override, since the background alone already conveys
              -- focus (see 'taskNameAttr'\/'taskTimeAttr' for how row text is styled instead).
            ...
            , (haskellLogoArrowAttr, (V.withStyle V.defAttr V.bold) `V.withForeColor` RGBColor 0x45 0x3a 0x62)
            , (haskellLogoLambdaAttr, (V.withStyle V.defAttr V.bold) `V.withForeColor` RGBColor 0x5e 0x50 0x86)
            , (haskellLogoEqualsAttr, V.defAttr `V.withForeColor` RGBColor 0x8f 0x4e 0x8b)
            , (moduleNameAttr, V.withStyle (V.defAttr `V.withForeColor` blue) V.bold)
            , (metadataAttr, V.withStyle (V.defAttr `V.withForeColor` magenta) V.bold)
            , (executeAttr, V.withStyle (V.defAttr `V.withForeColor` green) V.bold)
            , (nodeLabelAttr, V.withStyle V.defAttr V.bold)
            ]
