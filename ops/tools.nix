...
in {
  ...
  config = {
    ...
    cd $project
    ${profiledServerPkg}/bin/ghc-server --verbose $project +RTS $rts_opts &
    server_pid=$!
    cd -
    ...
    export resource_test_ext_deps=${extDeps}
    ${profiledServerPkg}/bin/gen-project production $project $depth $big_mods
    ...
    origdir=$PWD
    cd $project
    ${profiledServerPkg}/bin/ghc-server ''${feature_args[@]} $project +RTS -p -RTS &
    server_pid=$!
    cd $origdir
    ${profiledServerPkg}/bin/ghc-client $project --wait unitbig:metadata
