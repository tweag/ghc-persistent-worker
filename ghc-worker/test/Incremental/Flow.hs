...
import qualified Test.Run as Test
import Test.Run (defTest, defTestGhc, testSession)
...
restoreUnit state plans =
  withFrozenCallStack do
    testSession "restore unit" conf {Test.state = Just state}
  where
    conf = defTestGhc \ env -> do
      hsc_env <- getSession
      dflags0 <- getSessionDynFlags
      -- TODO this is also called in prepareMetadataSession, maybe it's superfluous
      --
      -- Also in loadCachedDepUnits, unit1 is determined to be missing in both invocations
      --
...
runMetadata desc state prog =
  testSession ("metadata " ++ desc) $ conf {Test.args, Test.state = Just state}
    where
      conf = defTest \ env -> do
        controlT \ lowerT ->
