...
rk unit modName = ResolvedModule (UnitName (Text.pack unit)) (mkModuleName modName)

-- | Helper to build a pending compile key.
pk :: String -> String -> TaskKey 'Pending
pk unit src = PendingSource (UnitName (Text.pack unit)) (toOsPath src)
...
runNodeDepsSpec spec = do
  let
    nameMap = Map.fromList [(stringToUnitId uid, UnitName (Text.pack n)) | (uid, n) <- spec.unitIds]
    srcMap = Map.fromList
...
runResolve spec =
  ...
  where
    ...
    newModules = resolveFromCachedUnit (UnitName (Text.pack spec.unitName)) (toOsPath "output") spec.cachedUnit
...
mkPriorModule unitName modName src =
  ( ModuleKey {unit = UnitName (Text.pack unitName), name = mkModuleName modName}
  , ModuleInfo {source = toOsPath src, deps = Set.empty, hiPath = mempty}
  )
