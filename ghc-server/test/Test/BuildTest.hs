...
import Types.Api (TaskTrigger (..), UnitName (..))
...
decisionActivated decisions =
  distinct [Text.unpack name.text ++ ":" ++ moduleNameString modName | DecisionActivated (ResolvedModule name modName) _ <- decisions]
...
decisionUpToDate decisions =
  distinct [Text.unpack name.text ++ ":" ++ moduleNameString modName | DecisionUpToDate (ResolvedModule name modName) _ _ <- decisions]
...
decisionDeduped decisions =
  distinct [Text.unpack name.text ++ ":" ++ moduleNameString modName | DecisionDeduped (ResolvedModule name modName) _ <- decisions]
...
prettyBuildResult label result =
  unlines $
    [label ++ ":",
     ...
     "  metadata errors:"]
    ++ ["    " ++ Text.unpack u.text ++ ": " ++ msg | (u, msg) <- result.metadataErrors]
    ++ ["  compile errors:"]
    ++ ["    " ++ Text.unpack u.text ++ ":" ++ show modName ++ ": " ++ msg | (u, modName, msg) <- result.compileErrors]
    ++ ["  execute errors:"]
    ++ ["    " ++ Text.unpack u.text ++ ":" ++ show modName ++ ": " ++ msg | (u, modName, msg) <- result.executeErrors]
...
assertCacheExists tp name =
  withFrozenCallStack do
    exists <- liftIO (cacheExists (unitCache tp.project (UnitName (Text.pack name))))
    assert exists
...
unitCache project name =
  case Map.lookup name project.units of
    ...
    Nothing -> error ("Unit not found: " ++ Text.unpack name.text)
...
test_uiRebuildRace =
  uiReproTest "repeated UI rebuild ('b', 'r', 'r') does not corrupt an intra-unit dependency" \ tp -> do
    ...
    chan <- liftIO newChan
    liftIO $ triggerTask (Just chan) cb tp.project TaskTrigger {target = Api.TargetProject, task = Api.Build False}
    liftIO $ awaitGenerationSettled _decisions 1
    liftIO $ triggerTask (Just chan) cb tp.project TaskTrigger {target = Api.TargetProject, task = Api.Build True}
...
-- | Resolve a unit in the build environment's project, failing the test when it is absent.
buildEnvUnit :: BuildEnv -> UnitName -> TestT IO Unit
buildEnvUnit buildEnv name =
  maybe (fail ("no such unit: " ++ show name)) pure (Map.lookup name buildEnv.project.units)
...
test_executeAfterCompile =
  execProjectTest "execute module after plain compile succeeds" \ tp -> do
    ...
    mresult <- liftIO (executeModuleTask buildEnv emptyBuildExt unit (mkModuleName "Main") 0 Nothing)
...
test_executeNonexistentModuleFails =
  execProjectTest "execute nonexistent module surfaces a failure, not a silent skip" \ tp -> do
    ...
    let name = UnitName "unit0"
    unit <- buildEnvUnit buildEnv name
    mresult <- liftIO (executeModuleTask buildEnv emptyBuildExt unit (mkModuleName "DoesNotExist") 0 Nothing)
    annotate ("executeModuleTask result: " ++ show mresult)
    case mresult of
