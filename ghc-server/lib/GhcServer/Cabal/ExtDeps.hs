...
module GhcServer.Cabal.ExtDeps where
...
scopedStoreDir :: IO OsPath
scopedStoreDir = do
  base <- defaultStoreDir
  pure (toOsPath (base ++ "-" ++ map sanitize libdir))
  where
...
resolvePackageDb :: Logger -> OsPath -> IO (Either String OsPath)
resolvePackageDb logger projectDir = do
  logger.info ("Building external dependencies in-process via cabal-install for " ++ fromOsPath projectDir)
  ...
  where
    build = do
      ...
      let
        ...
        cliConfig =
          mempty {
            projectConfigShared =
              mempty {
                projectConfigProjectDir = toFlag (fromOsPath projectDir),
                ...
                projectConfigStoreDir = toFlag (fromOsPath storeDir)
              },
      ...
      case [err | (_, Left err) <- Map.toList outcomes] of
        ...
        [] ->
          pure (toOsPath (storePackageDBPath ctx.cabalDirLayout.cabalStoreDirLayout buildCtx.elaboratedShared.pkgConfigCompiler))
...
ensureExtDepsDb :: BuildEnv -> Logger -> IO (Either String OsPath)
ensureExtDepsDb env logger =
  modifyMVar env.extDepsDb \case
    Just cached -> pure (Just cached, cached)
    Nothing -> do
      result <- resolvePackageDb logger env.projectRoot
