...
instance Show BuildEvent where
  show = \case
    MetadataSkipped name -> "MetadataSkipped " ++ Text.unpack name.text
    MetadataRan name -> "MetadataRan " ++ Text.unpack name.text
    ModuleCompiled name modName -> "ModuleCompiled " ++ Text.unpack name.text ++ ":" ++ moduleNameString modName
    ResolutionComputed name -> "ResolutionComputed " ++ Text.unpack name.text
