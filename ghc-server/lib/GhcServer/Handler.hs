...
import GhcServer.Data.Unit (ClientModule (..), Project (..), Unit (..), UnitCache (..))
...
validateUnit project name req =
  case project.units !? name of
    ...
    Nothing -> Left ("Unknown unit: " ++ Text.unpack name.text)
...
selectorRequest = \case
  ...
  SelModule moduleName -> UnitModules [ClientModule (Text.pack moduleName)]
  SelExecuteModule moduleName -> UnitExecuteModules [ClientModule (Text.pack moduleName)]
...
resolveTarget project (unitSel, sel) =
  ...
  in
    case unitSel of
      ...
      NamedUnit unit -> (: []) <$> validateUnit project (UnitName (Text.pack unit)) req
...
serverContext config = do
  ...
  -- exactly when 'startServerProcess' starts this process. 'emitLog' itself no-ops on a 'Nothing' channel, so
...
parseCleanTarget spec
  ...
  | otherwise =
    case Text.breakOn (Text.pack ":") spec of
      (unit, rest)
        | Text.null rest -> CleanUnit (UnitName unit)
        | otherwise -> CleanModule (UnitName unit) (mkModuleName (Text.unpack (Text.drop 1 rest)))

-- | Whether a 'TaskKey' is a compile\/execute task (not metadata) for some module other than the given one.
--
-- Phase-polymorphic so that it can be applied to the resolved completion maps and the pending
-- resolution map alike; pending keys never denote a specific module, so they are always retained.
notModuleTask :: ModuleKey -> TaskKey p -> Bool
notModuleTask key = \case
  ResolvedModule unit name -> ModuleKey {unit, name} /= key
  ExecuteModule unit name -> ModuleKey {unit, name} /= key
  _ -> True
...
-- | Drop every scheduler bookkeeping entry whose task key or module key is rejected by the given
-- predicates: the completion\/acceptance\/failure maps and the resolutions are filtered by the task
-- predicate, the accumulated module map and stale set by the module predicate.
retainTasks ::
...
retainTasks keepTask keepModule state =
  state
    ...
    , accepted = filterTasks state.accepted
    , failures = filterTasks state.failures
    , resolutions = filterTasks state.resolutions
    , ext =
      state.ext
        { moduleMap = Map.filterWithKey (\ k _ -> keepModule k) state.ext.moduleMap
        , stale = Set.filter keepModule state.ext.stale
        }
    }
  where
    filterTasks :: forall p v. Map (TaskKey p) v -> Map (TaskKey p) v
    filterTasks = Map.filterWithKey \ k _ -> keepTask k
...
resetSchedulerState build =
  modifySchedulerState build \ state ->
    state
      { completed = Map.empty
      , accepted = Map.empty
      , failures = Map.empty
...
invalidateUnitState build name =
  modifySchedulerState build (retainTasks (\ k -> taskUnit k /= name) (\ k -> k.unit /= name))
...
invalidateModuleState build unit modName =
  modifySchedulerState build (retainTasks (notModuleTask key) (/= key))
  where
    key = ModuleKey {unit, name = modName}
...
--
-- Dispatched through the unified 'Types.Api.Command' (the @Clean@ constructor) rather than its own gRPC
...
-- response directly, since that's now 'GhcServer.Grpc''s job (JSON-encoding a 'Types.Api.Response').
--
...
runClean :: Build -> BuildEnv -> Target -> IO (ApiResponse ())
runClean build env = \case
  TargetProject -> do
    _ <- filterM removeIfExists [env.outputDir, env.projectRoot </> cacheDirName]
    resetSchedulerState build
  ...
  TargetUnit {name} ->
    case Map.lookup name env.project.units of
      Nothing -> failure ("Unknown unit: " <> name.text)
      Just unit -> do
        _ <- filterM removeIfExists [unit.cache.dir, env.outputDir </> toOsPath (Text.unpack name.text)]
        invalidateUnitState build name
        modifyMVar_ env.diff (pure . Map.delete name)
        pure (ApiSuccess ())
  TargetModule {key = HomeModule {unit, name = uiName}} -> do
    let name = toGhcModuleName uiName
    case Map.lookup unit env.project.units of
      Nothing -> failure ("Unknown unit: " <> unit.text)
      Just _ -> do
        invalidateModuleState build unit name
        pure (ApiSuccess ())
  where
    failure msg = pure (ApiFailure msg)
