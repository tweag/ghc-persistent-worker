...
import GhcServer.Data.Unit (ClientModule (..), Project (..), Unit (..), UnitName (..), unitSources)
...
classifyBuildRequest env request = do
  ...
  where
    ...
    unitExecuteTasks eu =
      case (eu, Map.lookup (effectiveUnitName eu) env.project.units) of
        (Explicit name UnitExecute, Just unit) ->
          executeTasksFromSources name (unitSources unit)
    ...
    selectedSources unit mods =
      [src | src <- unitSources unit, takeBaseName (fromOsPath src) `elem` map (.string) mods]
