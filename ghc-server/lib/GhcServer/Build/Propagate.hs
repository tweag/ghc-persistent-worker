...
skipMetadata env name = do
  env.log.debug ("Skipping metadata (unchanged): " ++ Text.unpack name.text)
...
compile :: BuildExt -> BuildEnv -> Unit -> ModuleName -> Int -> IO (TaskResult String)
compile ext env unit modName requestId = do
  env.log.debugD ("Compile:" <+> O.text (Text.unpack unit.name.text) O.<> ":" O.<> ppr modName)
  logEvent env.events (ModuleCompiled unit.name modName)
  (result, _) <- compileSingleModule env unit modName cachedDeps requestId
  pure (taskResultFromErrors [(errUnit, errors) | (errUnit, _, errors) <- result])
  where
...
dispatchTask env ext task =
  case Map.lookup unitName env.project.units of
    ...
    Just unit -> dispatch unit
  where
    unitName = taskUnit task.key
...
computeResolutions cache env name _state =
  cache.loadUnit name >>= \case
    Nothing -> throwE ("computeResolutions: missing cached_unit.json for " <> name.text <> " after successful metadata")
    Just cu -> do
