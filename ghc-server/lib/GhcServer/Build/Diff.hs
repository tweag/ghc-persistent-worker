...
data UnitDiff =
  UnitDiff {
    -- | The unit the analysis was run for.
    unit :: Unit,
...
digestSource old src = do
  ...
  case Map.lookup key old of
    ...
    _ -> do
      hash <- getFileHash (fromOsPath src)
...
computeUnitDiff outputDir rebuild forceAll unit = do
  ...
  (changed, newDigests) <- digestSources old (unitSources unit)
  cached <- doesFileExist unit.cache.cachedUnitPath
  oldModules <- if cached
    then runExceptT (loadCachedUnit unit.cache) >>= \case
      Left err -> throwText err
      Right cu -> pure (maybe Map.empty (resolveFromCachedUnit unit.name outputDir) cu)
    else pure Map.empty
  pure UnitDiff {
    unit,
    changed,
...
-- | Map changed source paths to module keys via the refreshed module map's source paths.
changedModuleKeys :: Set OsPath -> Map ModuleKey ModuleInfo -> Set ModuleKey
changedModuleKeys changedPaths modules =
  Map.keysSet (Map.filter (\ info -> Set.member info.source changedPaths) modules)
...
commitDigests ::
  ...
  Set UnitName ->
  -- | Every module known to the build.
  ...
  Set ModuleKey ->
  IO ()
commitDigests diffs metaFailedUnits modules compiledModules stale =
  sequence_
    [ writeDigestRecord d.unit.cache (record d name)
    | (name, d) <- Map.toList diffs
    , not (Set.member name metaFailedUnits)
    ]
  where
    record d name = Map.union (Map.withoutKeys d.newDigests (failedFiles name)) d.oldDigests

    failedFiles name =
      Set.fromList
        [ fromOsPath info.source
        | key <- Set.toList stale
        , key.unit == name
        , not (Set.member key compiledModules)
        , Just info <- [Map.lookup key modules]
        ]
