...
import GhcServer.Data.Unit (Project (..), Unit (..), unitSources)
...
metadataArgs base outputDir cachedPlans buckHashes unit =
  base {
    ...
    ghcOptions =
      ...
      ++ depFlags
      ++ [
        "-this-unit-id", Text.unpack unit.name.text,
  ...
  where
    buildPlanPath = outDir </> [osp|build-plan.json|]

    incrementalStatePath = outDir </> [osp|incremental-state.json|]

    outDir = outputDir </> toOsPath (Text.unpack unit.name.text)

    depFlags = concatMap depFlag unit.depUnits

    depFlag dep = ["-package-id", Text.unpack dep.text]
...
runMetadata :: BuildEnv -> Unit -> IO ([(UnitName, String)], [String])
runMetadata buildEnv unit0 = do
  buildEnv.log.debug ("Metadata: " ++ Text.unpack name.text)
  logEvent buildEnv.events (MetadataRan name)
  withExtDeps unit0 (instrumentLogger buildEnv.instrChan (Text.unpack name.text ++ ":metadata") buildEnv.log)
  where
    name = unit0.name
    withExtDeps unit logger
      ...
      | otherwise =
        ensureExtDepsDb buildEnv logger >>= \case
          ...
          Right dbPath -> run (unit {ghcArgs = ["-package-db", fromOsPath dbPath] ++ unit.ghcArgs}) logger
    ...
    run unit logger = do
      ...
      let args = metadataArgs buildEnv.baseArgs buildEnv.outputDir (Just cachedPlans) buckHashes unit
          sourcePaths = fromOsPath <$> unitSources unit
          env = Env {
            log = logger,
    ...
    success unit cachedPlans args logger = do
      cacheResult <- case args.buildPlan of
        Nothing -> pure (Right ())
        Just buildPlan -> runExceptT (writeUnitCache logger unit.cache cachedPlans buildPlan args.ghcOptions)
      case cacheResult of
        Left err -> do
          captured <- logger.flush
          pure ([(name, "Cache write failed: " ++ Text.unpack err)], captured)
        Right () -> do
          captured <- logger.flush
          pure ([], captured)
