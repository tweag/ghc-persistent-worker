...
data Build =
  Build {
    ...
    commitLock :: MVar (),
    -- | Number of UI-triggered requests ('GhcServer.Grpc.triggerTask') whose 'scheduleBatch'\/'awaitBuild' pair
    -- is still in flight. Used to detect when the scheduler's queue has fully drained across every concurrent
    -- request, not just the one that happens to observe idleness first -- see 'GhcServer.Grpc.triggerTask'.
    inFlight :: TVar Int
  }
...
newBuild trace maxJobs taskTimeout buildEnv = do
  ...
  pure Build {scheduler, thread, env = buildEnv, commitLock, inFlight}
  ...
  modifyMVar_ cb.commitLock \ () -> do
    ...
    let
      ...
      compiledModules = Set.fromList
        [ ModuleKey {unit, name}
        | key@(ResolvedModule unit name) <- Map.keys completed
        , not (Map.member key failures)
        ]
    commitDigests diffs metaFailedUnits ext.moduleMap compiledModules ext.stale
  pure (collectBuildResult failures)
